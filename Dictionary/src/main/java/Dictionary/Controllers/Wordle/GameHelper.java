...
import static Dictionary.WordleGame.wordList;
import static Dictionary.WordleGame.secondaryWordList;
public class GameHelper {
    ...
    public void prepareGrid(GridPane gridPane) {
    ...
    public void prepareKeyboard(GridPane keyboardRow1, GridPane keyboardRow2, GridPane keyboardRow3) {
    ...
    /** Set text label at gridpane (row, col) to input */
    ...
    private void updateKeyboardColors(GridPane gridPane, GridPane keyboardRow1, GridPane keyboardRow2, GridPane keyboardRow3) {
        ...
        for (int i = 1; i <= MAX_COLUMN; i++) {
            ...
            String styleClass;
            String currentChar = String.valueOf(currentWord.charAt(i - 1));
            String winningChar = String.valueOf(winningWord.charAt(i - 1));

            if (contains(firstRowLetters, currentChar)) {
                keyboardLabel = getLabel(keyboardRow1, currentChar);
            } else if (contains(secondRowLetters, currentChar)) {
                keyboardLabel = getLabel(keyboardRow2, currentChar);
            } else if (contains(thirdRowLetters, currentChar)) {
                keyboardLabel = getLabel(keyboardRow3, currentChar);
            }

            if (currentChar.equals(winningChar)) {
                styleClass = "keyboardCorrectColor";
            } else if (winningWord.contains(currentChar)) {
    ...
    public void onKeyPressed(GridPane gridPane, GridPane keyboardRow1, GridPane keyboardRow2,
        ...
        try {
            ...
            alert.setTitle("Lỗi");
            alert.setContentText("Lỗi xử lí sự kiện");
            alert.show();
        }
    }

    /** Thêm chữ vào dòng khi người dùng nhập từ bàn phím*/
    private void onLetterPressed(GridPane gridPane, KeyEvent keyEvent) throws NullPointerException {
        // Nếu ô hiện tại chưa có chữ thì mới điền vào ô và khởi tạo hiệu ứng
        if (getLabelText(gridPane, CURRENT_ROW, CURRENT_COLUMN).isEmpty()) {
            ...
            firstScaleTransition.toXProperty().setValue(1.2);
            firstScaleTransition.fromYProperty().setValue(1);
            firstScaleTransition.toYProperty().setValue(1.2);
            ScaleTransition secondScaleTransition = new ScaleTransition(Duration.millis(100), label);
            secondScaleTransition.fromXProperty().setValue(1.2);
            secondScaleTransition.toXProperty().setValue(1);
            secondScaleTransition.fromYProperty().setValue(1.2);
    ...
    /** Xóa chữ ở dòng hiện tại khi backspace*/
    private void onBackspacePressed(GridPane gridPane) throws NullPointerException {
        // Con trỏ current_column ở ô ngay sau vị trí vừa điền chữ trừ khi vừa điền chữ ở ô cuối
        // Nếu ô hiện tại cột cuối thì nếu vị trí đó có chữ rồi thì xóa đi, còn không thì cần xóa ô trước đó
        if ((CURRENT_COLUMN == MAX_COLUMN || CURRENT_COLUMN == 1)
                && !getLabelText(gridPane, CURRENT_ROW, CURRENT_COLUMN).isEmpty()) {
            ...
            setLabelStyleClass(gridPane, CURRENT_ROW, CURRENT_COLUMN, "default-tile");
                ...
                || (CURRENT_COLUMN == MAX_COLUMN && getLabelText(gridPane, CURRENT_ROW, CURRENT_COLUMN).isEmpty())) {
            CURRENT_COLUMN--;
            setLabelText(gridPane, CURRENT_ROW, CURRENT_COLUMN, "");
            clearLabelStyleClass(gridPane, CURRENT_ROW, CURRENT_COLUMN);
            setLabelStyleClass(gridPane, CURRENT_ROW, CURRENT_COLUMN, "default-tile");
    ...
    /** Xử lí khi người dùng bấm Enter */
    ...
    private boolean isValidGuess(String guess) {
        return binarySearch(wordList, guess) || binarySearch(secondaryWordList, guess);
    }
